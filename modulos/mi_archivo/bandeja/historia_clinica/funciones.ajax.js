$(document).ready(function() {
	...
	$("body").on("click",".radicado_filtrar",function(event){
		$.ajax({
	...
	$(document).on('click', '#BtnDescargarPdfRecibido', function (event){
		...
		if($("#id_ruta").val() == 0){
			...
			$.ajax({
				...
				type: 'POST',
				data: 'accion=RECIBIDOS_DESCARGAR&id_radica='+$("#id_radica").val()+'&id_ruta='+$("#id_ruta").val()+'&RutaDonwload='+$("#RutaDonwload").val(),
				beforeSend: function(){
