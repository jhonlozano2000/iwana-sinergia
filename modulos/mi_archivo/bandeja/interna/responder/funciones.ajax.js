$(document).ready(function () {
	...
	$('#BtnRadicar').click(function(){
		...
		if($('#multi').val() == null) {
			...
			var dataString = 'accion='+$('#accion').val()+'&id_radica_respuesta='+$('#id_radica_respuesta').val()+'&destinatarios='+$('#multi').val()+'&copia='+$('#multi1').val()+'&asunto='+$('#asunto').val()+'&texto='+postBody+'&fec_venci='+$('#fec_venci').val()+'&id_serie='+$('#id_serie').val()+'&id_subserie='+$('#id_subserie').val()+'&id_tipodoc='+$('#id_tipodoc').val();

			$.ajax({
				type: 'POST',
				url: '../radicar/acciones.ajax.php',
				data: dataString,
				cache: true,
				beforeSend: function(){
