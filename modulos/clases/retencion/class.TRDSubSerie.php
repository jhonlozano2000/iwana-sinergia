 <?php
	class SubSerie
	{
		...
		public function Gestionar()
		{
			...
			try {
				if ($this->Accion == 'INSERTAR') {
					$Sql = "INSERT INTO archivo_trd_subserie(cod_subserie, nom_subserie, acti) 
				...
				if ($this->Accion == 'EDITAR') {
					...
					$Sql = "UPDATE archivo_trd_subserie SET cod_subserie = :cod_subserie, nom_subserie = :nom_subserie, 
			 				acti = :acti
				...
				if ($this->Accion == 'ELIMINAR') {
					$Sql = "DELETE FROM archivo_trd_subserie 
	 			WHERE id_subserie = :id_subserie";
		...
		public static function Listar($Accion, $IdSubSerie, $IdDocu, $Cod, $Nom)
		{
			...
			$Sql = "SELECT * FROM archivo_trd_subserie ";
			...
			try {
				...
				if ($Accion == 1) {
		        	...
		        	/******************************************************************************************/
					$Sql = "SELECT archivo_trd_subserie.id_subserie, archivo_trd_subserie.cod_subserie,
								archivo_trd_subserie.nom_subserie, archivo_trd_subserie.acti AS acti_subserie,
								archivo_trd_tipo_docu.nom_tipodoc, archivo_trd_subserie_docu.acti AS acti_docu
		        	...
		        	/******************************************************************************************/
					$Sql = "SELECT archivo_trd_tipo_docu.id_tipodoc, archivo_trd_subserie_docu.acti, archivo_trd_tipo_docu.nom_tipodoc
							FROM archivo_trd_subserie_docu
								INNER JOIN archivo_trd_tipo_docu ON (archivo_trd_subserie_docu.id_tipodoc = archivo_trd_tipo_docu.id_tipodoc) 
							WHERE archivo_trd_subserie_docu.id_subserie = :id_subserie";
					$Instruc = $conexion->prepare($Sql);
					$Instruc->bindParam(':id_subserie', $IdSubSerie, PDO::PARAM_INT);
					$Instruc->execute() or die(print_r($Instruc->errorInfo() . " - " . $Sql, true));
				} elseif ($Accion == 7) {
				...
				echo 'Ha surgido un error y no se puede ejecutar la consulta, Sub Series -> Acción: ' . $Accion . " - " . $e->getMessage();
				exit;
			}
		}
