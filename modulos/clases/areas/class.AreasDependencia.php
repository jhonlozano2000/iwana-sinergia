	...
	public function Gestionar()
	{
		...
		try {
			if ($this->Accion == 'INSERTAR') {
				$Sql = "INSERT INTO areas_dependencias(cod_depen, cod_corres, nom_depen, observa, acti)
	...
	public static function Listar($Accion, $Id, $Nom, $CodDepen, $CodCorre)
	{
		...
		try {
			...
			if ($Accion == 1) {
		        ...
		        /******************************************************************************************/
				$Sql = "SELECT * FROM areas_dependencias ORDER BY nom_depen ASC";
				$Instruc = $conexion->prepare($Sql);
				$Instruc->execute() or die(print_r($Instruc->errorInfo() . " - " . $Sql, true));
			} elseif ($Accion == 2) {
				/******************************************************************************************/
				/*	LISTO UN REGISTROS
				/******************************************************************************************/
				$Sql = "SELECT * FROM areas_dependencias WHERE id_depen = :id_depen";
				$Instruc = $conexion->prepare($Sql);
				...
				/*	LISTO POR NOMBRE
				/******************************************************************************************/
				$Sql = "SELECT * FROM areas_dependencias WHERE nom_depen = :nom_depen";
				$Instruc = $conexion->prepare($Sql);
				...
				/******************************************************************************************/
				$Sql = "SELECT * FROM areas_dependencias WHERE cod_depen = :cod_depen";
				$Instruc = $conexion->prepare($Sql);
				...
				/*	LISTO POR CODIGO DE LA DEPENDENCIA
				/******************************************************************************************/
				$Sql = "SELECT * FROM areas_dependencias WHERE cod_corres = :cod_corres";
				$Instruc = $conexion->prepare($Sql);
				...
				/******************************************************************************************/
				$Sql = "SELECT * FROM areas_dependencias WHERE acti = 1";
				$Instruc = $conexion->prepare($Sql);
				$Instruc->execute() or die(print_r($Instruc->errorInfo() . " - " . $Sql, true));
			}
