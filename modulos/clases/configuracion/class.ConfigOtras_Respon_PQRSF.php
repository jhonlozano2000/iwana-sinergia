<?php
class ConfigOtrasResponsablePQRSF
{
	private $Accion;
	private $IdDepen;
	...
	public function Gestionar()
	{
		...
		try {
			if ($this->Accion == 'INSERTAR_RESPONSABLE') {
				$Sql = "INSERT INTO config_otras_responsables_pqrsf(id_funcio_deta, id_depen, id_serie, id_subserie, id_tipodoc) 
				...
				$Sql = "DELETE FROM config_otras_responsables_pqrsf WHERE id_funcio_deta= :id_funcio_deta";
				$Instruc = $conexion->prepare($Sql);
				$Instruc->bindParam(':id_funcio_deta', $this->IdFuncioDeta, PDO::PARAM_INT);
			}
			...
			echo 'Ha surgido un error y no se puede ejecutar la consulta, Configuracion->Otras->Responsable PQRS, Accion: ' . $this->Accion . " - " . $e->getMessage();
			exit;
		}
	}
		...
		try {
			if ($Accion === 1) {
				$Sql = "SELECT respon.id_funcio_deta, depen.nom_depen, serie.nom_serie, subserie.nom_subserie, documen.nom_tipodoc, 
						...
						FROM config_otras_responsables_pqrsf AS respon
						...
						ORDER BY depen.nom_depen, funcio.nom_funcio, funcio.ape_funcio";
	...
	public static function Buscar($Accion, $IdFuncioDeta)
	{
		...
		try {
			if ($Accion == 1) {
				$Sql = "SELECT *
						FROM config_otras_responsables_pqrsf
