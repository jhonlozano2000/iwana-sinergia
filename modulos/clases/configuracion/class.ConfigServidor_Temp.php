...
class ServidorTemp
{
	...
	public function __construct(
		...
		$this->Contra        = $Contra;
		$this->Ruta          = $Ruta;
		$this->TipoCorrespon = $TipoCorrespon;
	...
	public function get_TipoCorrespon()
	{
		return $this->TipoCorrespon;
	}
	...
	public function set_TipoCorrespon($TipoCorrespon)
	{
		$this->TipoCorrespon = $TipoCorrespon;
	}
	...
	public function Gestionar()
	{
		...
		try {
			if ($this->Accion == 'INSERTAR') {
				$Sql = "INSERT INTO config_rutas_archi_temp(ip, ruta, usua, contra, tipo_correspon, observa) 
						VALUES(:ip, :ruta, :usua, :contra, :tipo_correspon, :observa)";
				...
				$Instruc->bindParam(':tipo_correspon', $this->TipoCorrespon, PDO::PARAM_STR);
				$Instruc->bindParam(':observa', $this->Observa, PDO::PARAM_STR);
			} elseif ($this->Accion == 'EDITAR') {
				$Sql = "UPDATE config_rutas_archi_temp
						SET ip = :ip, ruta = :ruta, usua = :usua, contra = :contra, tipo_correspon = :tipo_correspon,
				...
				$Sql = "DELETE FROM config_rutas_archi_temp
						WHERE id_ruta = :id_ruta";
	...
	public static function Buscar($Accion, $IdRuta, $Ruta, $TipoCorrespon)
	{
		...
		try {
			...
			if ($Accion == 1) {
				...
				/*  LISTO TODOS LOS REGISTROS
		        /******************************************************************************************/
				$Sql = "SELECT * FROM config_rutas_archi_temp ORDER BY ruta ASC";
				...
				/*  LISTO UN REGISTROS
		        /******************************************************************************************/
				$Sql = "SELECT * FROM config_rutas_archi_temp WHERE id_ruta = :id_ruta";
				...
				/*  BUSCO LA RUTA QUE ESTE ACTIVA DE UNA DEPENDENCIA
		        /******************************************************************************************/
				$Sql = "SELECT * FROM config_rutas_archi_temp WHERE acti = 1";
				$Instruc = $conexion->prepare($Sql);
				$Instruc->execute() or die(print_r($Instruc->errorInfo() . " - " . $Sql, true));
			} elseif ($Accion == 4) {
				/******************************************************************************************/
				...
				/******************************************************************************************/
				/*  BUSCO LA RUTA ACTIVA PARA UN TIPO DE CORRESPONDENCIA
		        /******************************************************************************************/

				$Sql = "SELECT * 
		        		FROM config_rutas_archi_temp 
		        		WHERE acti = 1 AND tipo_correspon = :tipo_correspon";
				$Instruc = $conexion->prepare($Sql);
				$Instruc->bindParam(":tipo_correspon", $TipoCorrespon, PDO::PARAM_STR);
				$Instruc->execute() or die(print_r($Instruc->errorInfo() . " - " . $Sql, true));
			}
