<?php
class RadicadoRecibidoListarCompartido{
    private $Accion;
    private $IdRadica;
    ...
    public function Listar(){
        ...
        try{
            ...
            if($this->Accion == 1){
                ...
                /******************************************************************************************/
                $Sql = "SELECT DISTINCT `radi`.`id_radica`, `radi`.`fechor_radica`, `radi`.`fec_docu`, `radi`.`fec_venci`, `radi`.`asunto`, `radi`.`impri_rotu`, 
                        ...
                        FROM `archivo_radica_recibidos_responsa` AS `ra_respon`
                            ...
                            INNER JOIN `archivo_radica_recibido_compartidos` AS `radi_compar` ON (`radi`.`id_radica` = `radi_compar`.`id_radica`)
                ...
                $Sql = "SELECT DISTINCT `radi`.`id_radica`, `radi`.`fechor_radica`, `radi`.`fec_docu`, `radi`.`fec_venci`, `radi`.`asunto`, `radi`.`impri_rotu`, 
                            ...
                            `forma_llega`.`nom_formaenvi` AS `nom_forma_llega`, `forma_llega`.`requie_digital`, `radi`.`autoriza`
                        FROM `archivo_radica_recibidos_responsa` AS `ra_respon`
                            INNER JOIN `archivo_radica_recibidos` AS `radi` ON (`ra_respon`.`id_radica` = `radi`.`id_radica`)
                            INNER JOIN `gene_terceros_contac` AS `terce_contac` ON (`radi`.`id_remite` = `terce_contac`.`id_tercero`)
                            INNER JOIN `segu_usua` AS `usua_radica` ON (`radi`.`id_usua_regis` = `usua_radica`.`id_usua`)
                            ...
                            INNER JOIN `config_formaenvio` AS `forma_llega` ON (`radi`.`id_forma_llegada` = `forma_llega`.`id_formaenvio`)
                            INNER JOIN `archivo_radica_recibido_compartidos` AS `radi_compar` ON (`radi`.`id_radica` = `radi_compar`.`id_radica`)
                        WHERE (`radi`.`transferido` = 0 AND `radi`.`autoriza` = 1 AND `ra_respon`.`respon` = 1 
                            AND `radi_compar`.`id_funcio_deta_destino` = :id_funcio)
            ...
            if($this->Accion == 5){
                ...
                $Sql = "SELECT `radi`.`id_radica`, `radi`.`fechor_radica`, `radi`.`fec_docu`, `radi`.`fec_venci`, `radi`.`asunto`, `radi`.`impri_rotu`, 
                        ...
                        FROM `archivo_radica_recibidos_responsa` AS `ra_respon`
                            ...
                            INNER JOIN `gene_funcionarios` AS `funcio_radica` ON (`usua_radica`.`id_funcio` = `funcio_radica`.`id_funcio`)
                            INNER JOIN `config_formaenvio` AS `forma_llega` ON (`radi`.`id_forma_llegada` = `forma_llega`.`id_formaenvio`)
                        WHERE (`radi`.`transferido` = 0 AND `ra_respon`.`respon` = 1 ".self::Generar_Query_Correspondencia($this->Criterio3).")
                        ORDER BY `radi`.`fechor_radica` DESC
                        LIMIT 0, 50";
                ...
                $Sql = "SELECT `radi`.`id_radica`, `radi`.`fechor_radica`, `radi`.`fec_docu`, `radi`.`fec_venci`, `radi`.`asunto`, `radi`.`impri_rotu`, 
                        ...
                        FROM `archivo_radica_recibidos_responsa` AS `ra_respon`
                            ...
                            INNER JOIN `config_formaenvio` AS `forma_llega` ON (`radi`.`id_forma_llegada` = `forma_llega`.`id_formaenvio`)
                        WHERE (`radi`.`transferido` = 0 AND `radi`.`autoriza` = 1 AND `ra_respon`.`respon` = 1 AND `depen`.`id_depen` = ? 
                            AND `funcio_deta`.`id_oficina` = ? ".self::Generar_Query_Correspondencia($this->Criterio3).")
                        ORDER BY `radi`.`fechor_radica` DESC
                        LIMIT 0, 50";
            ...
            if($this->Accion == 1){
                ...
                $Sql = "SELECT `radi`.`id_radica`
                        ...
                        WHERE (`radi`.`transferido` = 0 AND `radi`.`autoriza` = 1 AND `ra_respon`.`respon` = 1 AND `depen`.`id_depen` = ? AND `funcio_deta`.`id_oficina` = ?
                            OR".self::Generar_Query_Correspondencia($this->Criterio3).")";
