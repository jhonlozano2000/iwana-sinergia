<?php
class RadicadoRecibidoProntoAVence{
    private $Accion;
    private $Buscar;
    private $IdRadica;
        ...
        try{
            ...
            if($this->Accion == 1){
                ...
                $Sql = "SELECT `radi`.`id_radica`, `radi`.`fechor_radica`, `radi`.`fec_docu`, `radi`.`fec_venci`, `radi`.`asunto`, `radi`.`impri_rotu`,
                            ...
                            `forma_llega`.`nom_formaenvi` AS `nom_forma_llega`, `forma_llega`.`requie_digital`, `radi`.`autoriza`, `radi`.`respondido`, `radi`.`pase`, `radi`.`autoriza`
                ...
                $Instruc -> bindParam(":Limite2", $this->Limite2, PDO::PARAM_INT);
                $Instruc->execute() or die(print_r($Instruc->errorInfo()." - ".$Sql, true));
            }elseif($this->Accion == 13){
                /******************************************************************************************/
                /*  LISTO TODA LA CORRESPONDENCIA PARA NOTIFICACION POR CORREO ELECTRONICO
                /******************************************************************************************/
                $Sql = "SELECT `radi`.`id_radica`, `radi`.`fechor_radica`, `radi`.`fec_docu`, `radi`.`fec_venci`, `radi`.`asunto`, `radi`.`impri_rotu`,
                            `radi`.`digital`, `radi`.`requie_respues`, `radi`.`autoriza`, `funcio`.`nom_funcio`, `funcio`.`ape_funcio`,
                            `ofi`.`nom_oficina`, `depen`.`nom_depen`, `terce_contac`.`nom_contac`, `terce_contac`.`cargo`, `terce_empre`.`nit_empre`,
                        ...
                        FROM `archivo_radica_recibidos_responsa` AS `ra_respon`
                            ...
                            INNER JOIN `gene_funcionarios` AS `funcio_radica` ON (`usua_radica`.`id_funcio` = `funcio_radica`.`id_funcio`)
                            INNER JOIN `config_formaenvio` AS `forma_llega` ON (`radi`.`id_forma_llegada` = `forma_llega`.`id_formaenvio`)
                        WHERE (`radi`.`transferido` = 0 AND `ra_respon`.`respon` = 1 AND `radi`.`requie_respues` = 1 AND `radi`.`respondido` = 0 AND `radi`.`autoriza` = 1)
                        ORDER BY `radi`.`id_radica` DESC, `radi`.`fechor_radica` ASC";
    ...
    public function Filtro(){
        ...
        try{
            ...
            if($this->Accion == 5){
                ...
                $Sql = "SELECT `radi`.`id_radica`, `radi`.`fechor_radica`, `radi`.`fec_docu`, `radi`.`fec_venci`, `radi`.`asunto`, `radi`.`impri_rotu`,
                        ...
                        FROM `archivo_radica_recibidos_responsa` AS `ra_respon`
                            ...
                            INNER JOIN `config_formaenvio` AS `forma_llega` ON (`radi`.`id_forma_llegada` = `forma_llega`.`id_formaenvio`)
                        WHERE (`radi`.`transferido` = 0 AND `radi`.`autoriza` = 1 AND `ra_respon`.`respon` = 1 AND `depen`.`id_depen` = :id_depen
                            AND `funcio_deta`.`id_oficina` = :id_oficina ".self::Generar_Query_Correspondencia($this->Buscar, $this->IdRadica, $this->IdTercero, $this->IdFuncioDeta, $this->Asunto, $this->QueContenga, $this->IdDepen, $this->IdOfi).")
                        ORDER BY `radi`.`fechor_radica` DESC
                        LIMIT :Limite1, :Limite2";
            ...
            echo 'Ha surgido un error y no se puede ejecutar la consulta Filtro.'.$e->getMessage();
            exit;
        }
    }
            ...
            if($this->Accion == 1){
                ...
                $Sql = "SELECT `radi`.`id_radica`
                        FROM `archivo_radica_recibidos_responsa` AS `ra_respon`
                            ...
                            INNER JOIN `gene_funcionarios` AS `funcio_radica` ON (`usua_radica`.`id_funcio` = `funcio_radica`.`id_funcio`)
                            INNER JOIN `config_formaenvio` AS `forma_llega` ON (`radi`.`id_forma_llegada` = `forma_llega`.`id_formaenvio`)
                        WHERE (`radi`.`transferido` = 0 AND `radi`.`autoriza` = 1 AND `ra_respon`.`respon` = 1 AND `depen`.`id_depen` = ? AND `funcio_deta`.`id_oficina` = ?
                            OR".self::Generar_Query_Correspondencia($this->Buscar, $this->IdRadica, $this->IdTercero, $this->IdFuncioDeta, $this->Asunto, $this->QueContenga, $this->IdDepen, $this->IdOfi).")";
