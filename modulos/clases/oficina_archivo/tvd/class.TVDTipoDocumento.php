...
class TipoDocumentoTVD{
	...
	public function Gestionar() {
        ...
        try{
			if($this->Accion == 'INSERTAR'){
				$Sql = "INSERT INTO archivo_tvd_tipo_docu (nom_tipodoc, observa, acti)
				...
				$Sql = "UPDATE archivo_tvd_tipo_docu
				...
				$Sql = "DELETE FROM archivo_tvd_tipo_docu WHERE id_tipodoc = :id_tipodoc";
	...
	public static function Buscar($Accion, $Id, $Nom) {
		...
		try{
			if($Accion == 1){
				$Sql = "SELECT * FROM archivo_tvd_tipo_docu WHERE id_tipodoc = ".$Id;
				$Instruc = $conexion->prepare($Sql);
                $Instruc -> bindParam(':id_tipodoc', $Id, PDO::PARAM_INT);
                $Instruc->execute() or die(print_r($Instruc->errorInfo()." - ".$Sql, true));
			}elseif($Accion == 2){
				$Sql = "SELECT * FROM archivo_tvd_tipo_docu WHERE nom_tipodoc = '".$Nom."'";
				$Instruc = $conexion->prepare($Sql);
