...
class TVD{
    ...
    public function __construct($Accion=null, $IdTVD = null, $IdDependencia = null, $IdOficina =null, $IdSerie = null, $IdSubSerie = null, $ag = null,
    ...
    public function set_IdTVD($IdTVD){
        return $this-> IdTVD = $IdTVD;
    }
    ...
    public function Gestionar(){
        ...
        try{
            ...
            if($this->Accion == 'INSERTAR'){
                ...
                $Sql = "INSERT INTO `archivo_tvd`(`id_depen`, `id_oficina`, `id_serie`,`id_subserie`,`ag`,`ac`,`ct`,`e`,`dm`,`s`,`observa`,`acti`)
                ...
                $Sql = "UPDATE `archivo_tvd`
                        SET `id_tvd` = :id_tvd, `id_depen` = :id_depen, `id_serie` = :id_serie, `id_subserie` = :id_subserie, `ag` = :ag,
                ...
                $Instruc -> bindParam(':id_tvd', $this->IdTVD, PDO::PARAM_INT);
    ...
    public static function Listar($Accion, $Id, $IdDepen, $IdSerie, $IdSubSerie){
        ...
        try{
            ...
            $Sql = "SELECT TRD.id_tvd, Depen.id_depen, Depen.cod_depen, Depen.cod_corres, Depen.nom_depen, Seri.id_serie, Seri.cod_serie,
                    ...
                    FROM archivo_tvd AS TRD
                        INNER JOIN archivo_tvd_dependencias AS Depen ON (TRD.id_depen = Depen.id_depen)
                        INNER JOIN archivo_tvd_series AS Seri ON (TRD.id_serie = Seri.id_serie)
                        INNER JOIN archivo_tvd_subserie AS Sub ON (TRD.id_subserie = Sub.id_subserie)
                        LEFT JOIN archivo_tvd_subserie_docu AS SubDeta ON (SubDeta.id_subserie = Sub.id_subserie)
                        LEFT JOIN archivo_tvd_tipo_docu AS TipDoc ON (SubDeta.id_tipodoc = TipDoc.id_tipodoc) ";

            if($Accion == 1){
                /******************************************************************************************/
                /*  LISTO TODOS LOS REGISTROS
                /******************************************************************************************/
                $Sql = "SELECT DISTINCT archivo_tvd.id_tvd, archivo_tvd_dependencias.id_depen, archivo_tvd_dependencias.cod_depen, archivo_tvd_dependencias.cod_corres,
                            archivo_tvd_dependencias.nom_depen, archivo_tvd_series.id_serie, archivo_tvd_series.cod_serie, archivo_tvd_series.nom_serie,
                            archivo_tvd_series.acti AS acti_serie, archivo_tvd_subserie.id_subserie, archivo_tvd_subserie.cod_subserie,
                            archivo_tvd_subserie.nom_subserie, archivo_tvd_subserie.acti AS acti_subserie, archivo_tvd.ag, archivo_tvd.ac,
                            archivo_tvd.ct, archivo_tvd.e, archivo_tvd.dm, archivo_tvd.s, archivo_tvd.observa, archivo_tvd.acti AS acti_trd
                        FROM archivo_tvd
                            INNER JOIN archivo_tvd_dependencias ON (archivo_tvd.id_depen = archivo_tvd_dependencias.id_depen)
                            INNER JOIN archivo_tvd_series ON (archivo_tvd.id_serie = archivo_tvd_series.id_serie)
                            INNER JOIN archivo_tvd_subserie ON (archivo_tvd.id_subserie = archivo_tvd_subserie.id_subserie)
                        ORDER BY archivo_tvd_dependencias.nom_depen ASC";
                ...
                $Sql.="WHERE TRD.id_tvd = :id_tvd";

                $Instruc = $conexion->prepare($Sql);
                $Instruc -> bindParam(':id_tvd', $Id, PDO::PARAM_INT);
                $Instruc->execute() or die(print_r($Instruc->errorInfo()." - ".$Sql, true));
            }elseif($Accion == 3){
                /******************************************************************************************/
                ...
                /******************************************************************************************/
                $Sql = "SELECT `archivo_tvd`.`id_tvd`, `archivo_tvd_dependencias`.`cod_depen`, `archivo_tvd_series`.`id_serie`, `archivo_tvd_series`.`cod_serie`, `archivo_tvd_series`.`nom_serie`,
                            `archivo_tvd_subserie`.`id_subserie`, `archivo_tvd_subserie`.`cod_subserie`, `archivo_tvd_subserie`.`nom_subserie`, `archivo_tvd`.`ag`,
                            `archivo_tvd`.`ac`, `archivo_tvd`.`ct`, `archivo_tvd`.`e`, `archivo_tvd`.`dm`, `archivo_tvd`.`s`, `archivo_tvd`.`observa`, `archivo_tvd`.`acti`
                        FROM `archivo_tvd`
                            INNER JOIN `archivo_tvd_series` ON (`archivo_tvd`.`id_serie` = `archivo_tvd_series`.`id_serie`)
                            INNER JOIN `archivo_tvd_subserie` ON (`archivo_tvd`.`id_subserie` = `archivo_tvd_subserie`.`id_subserie`)
                            INNER JOIN `archivo_tvd_dependencias` ON (`archivo_tvd`.`id_depen` = `archivo_tvd_dependencias`.`id_depen`)
                        WHERE (`archivo_tvd`.`id_depen` = :id_depen)
                        ORDER BY `archivo_tvd_series`.`nom_serie`, `archivo_tvd_subserie`.`nom_subserie` ASC";
                ...
                $Sql = "SELECT DISTINCT `tipo_docu`.`id_tipodoc`, `tipo_docu`.`nom_tipodoc`, `trd`.`id_subserie`
                        FROM `archivo_tvd` AS `trd`
                            INNER JOIN `archivo_tvd_subserie` AS `sub_serie` ON (`trd`.`id_subserie` = `sub_serie`.`id_subserie`)
                            INNER JOIN `archivo_tvd_subserie_docu` AS `sub_serie_deta` ON (`sub_serie_deta`.`id_subserie` = `sub_serie`.`id_subserie`)
                            INNER JOIN `archivo_tvd_tipo_docu` AS `tipo_docu` ON (`sub_serie_deta`.`id_tipodoc` = `tipo_docu`.`id_tipodoc`)
                ...
                /******************************************************************************************/
                $Sql = "SELECT DISTINCT `archivo_tvd_subserie`.`id_subserie`, `archivo_tvd_subserie`.`cod_subserie`, `archivo_tvd_subserie`.`nom_subserie`
                        FROM `archivo_tvd`
                            INNER JOIN `archivo_tvd_subserie` ON (`archivo_tvd`.`id_subserie` = `archivo_tvd_subserie`.`id_subserie`)
                        WHERE (`archivo_tvd_subserie`.`acti` = 1 AND `archivo_tvd`.`acti` = 1 AND `archivo_tvd`.`id_depen` = :id_depen
                            AND `archivo_tvd`.`id_serie` = :id_serie)";
                ...
                /******************************************************************************************/
                 $Sql = "SELECT `archivo_tvd`.`id_tvd`, `archivo_tvd_dependencias`.`cod_depen`, `areas_oficinas`.`cod_oficina`, `archivo_tvd_series`.`id_serie`,
                            `archivo_tvd_series`.`cod_serie`, `archivo_tvd_series`.`nom_serie`, `archivo_tvd_subserie`.`id_subserie`,
                            `archivo_tvd_subserie`.`cod_subserie`, `archivo_tvd_subserie`.`nom_subserie`, `archivo_tvd`.`ag`, `archivo_tvd`.`ac`,
                            `archivo_tvd`.`ct`, `archivo_tvd`.`e`, `archivo_tvd`.`dm`, `archivo_tvd`.`s`, `archivo_tvd`.`observa`, `archivo_tvd`.`acti`
                        FROM `archivo_tvd`
                            INNER JOIN `archivo_tvd_series` ON (`archivo_tvd`.`id_serie` = `archivo_tvd_series`.`id_serie`)
                            INNER JOIN `archivo_tvd_subserie` ON (`archivo_tvd`.`id_subserie` = `archivo_tvd_subserie`.`id_subserie`)
                            INNER JOIN `archivo_tvd_dependencias` ON (`archivo_tvd`.`id_depen` = `archivo_tvd_dependencias`.`id_depen`)
                            LEFT JOIN `areas_oficinas` ON (`archivo_tvd`.`id_oficina` = `areas_oficinas`.`id_oficina`)
                        WHERE (`archivo_tvd`.`id_depen` = :id_depen AND `archivo_tvd`.`id_oficina` = :id_oficina)
                        ORDER BY `archivo_tvd_series`.`nom_serie`, `archivo_tvd_subserie`.`nom_subserie` ASC";
                ...
                $Oficina = "";
                if($IdSerie != ""){
                    $Oficina = "AND `archivo_tvd`.`id_oficina` = :id_oficina";
                }else{
                    $Oficina = "";
                }

                $Sql = "SELECT DISTINCT `archivo_tvd_series`.`id_serie`, `archivo_tvd_series`.`cod_serie`, `archivo_tvd_series`.`nom_serie`
                        FROM `archivo_tvd`
                            INNER JOIN `archivo_tvd_series` ON (`archivo_tvd`.`id_serie` = `archivo_tvd_series`.`id_serie`)
                            INNER JOIN `archivo_tvd_subserie` ON (`archivo_tvd`.`id_subserie` = `archivo_tvd_subserie`.`id_subserie`)
                            INNER JOIN `archivo_tvd_dependencias` ON (`archivo_tvd`.`id_depen` = `archivo_tvd_dependencias`.`id_depen`)
                            LEFT JOIN `areas_oficinas` ON (`archivo_tvd`.`id_oficina` = `areas_oficinas`.`id_oficina`)
                        WHERE (`archivo_tvd`.`id_depen` = :id_depen ". $Oficina.")
                        ORDER BY `archivo_tvd_series`.`nom_serie` ASC";
                ...
                $Sql = "SELECT DISTINCT `archivo_tvd_subserie`.`id_subserie`, `archivo_tvd_subserie`.`cod_subserie`, `archivo_tvd_subserie`.`nom_subserie`
                        FROM `archivo_tvd`
                            INNER JOIN `archivo_tvd_subserie` ON (`archivo_tvd`.`id_subserie` = `archivo_tvd_subserie`.`id_subserie`)
                        WHERE (`archivo_tvd_subserie`.`acti` = 1 AND `archivo_tvd`.`acti` = 1 AND `archivo_tvd`.`id_depen` = :id_depen
                            AND `archivo_tvd`.`id_oficina` = :id_oficina AND `archivo_tvd`.`id_serie` = :id_serie)";
    ...
    public static function Buscar($Accion, $Id, $IdDepen, $IdSerie, $IdSubSerie) {
        ...
        try{
            ...
            if($Result){
                return new self("", $Result['id_tvd'], $Result['id_depen'], $Result['id_serie'],  $Result['id_subserie'], $Result['ag'],
