	...
	public function Gestionar(){
        ...
        try{
			if($this->Accion == 'Insertar'){
				$Sql = "INSERT INTO archivo_tvd_series(cod_serie, nom_serie, observa, acti) 
				...
				$Sql = "DELETE FROM archivo_tvd_series WHERE id_serie = :id_serie";
				$Instruc = $conexion->prepare($Sql);
                $Instruc -> bindParam(':id_serie', $this->IdSerie, PDO::PARAM_INT);
   	...
   	public static function Listar($Accion, $IdSerie, $Cod, $Nom) {
		...
		$Sql = "SELECT * FROM archivo_tvd_series ";
		...
		try{
			...
			if($Accion == 1){
		        ...
		        /******************************************************************************************/
		        $Sql = "SELECT archivo_tvd_series.cod_serie, archivo_tvd_series.nom_serie, archivo_tvd_series.acti AS acti_serie
			                , archivo_tvd_subserie.cod_subserie, archivo_tvd_subserie.nom_subserie, archivo_tvd_subserie.acti as acti_subserie
			            FROM archivo_trd
			                INNER JOIN archivo_tvd_series ON (archivo_trd.id_serie = archivo_tvd_series.id_serie)
			                INNER JOIN archivo_tvd_subserie ON (archivo_trd.id_subserie = archivo_tvd_subserie.id_subserie)
			        	ORDER BY archivo_tvd_series.cod_serie ASC, archivo_tvd_series.nom_serie ASC, archivo_tvd_subserie.cod_subserie ASC, 
			        		archivo_tvd_subserie.nom_subserie ASC";
		        $Instruc = $conexion->prepare($Sql);
				$Instruc -> execute() or die(print_r($Instruc -> errorInfo()." - ".$Sql, true));
		    }
