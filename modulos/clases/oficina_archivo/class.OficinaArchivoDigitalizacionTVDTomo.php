...
class DigitalizacioTVDTomo{
	...
	public function Gestionar(){
		...
		try{
			if($this->Accion == 'INSERTAR_TOMO'){
				$Sql = "INSERT INTO `archivo_digitales_tvd_tomos`(`id_digital`, `nom_tomo`)
				...
				$Sql = "UPDATE archivo_digitales_tvd_tomos SET id_digital = :id_digital, nom_tomo = :nom_tomo
			...
			if($Accion == 1){
				...
				*********************************************************************************************/
				$Sql = "SELECT COUNT(id_tomo) AS 'TotalTomos' 
						FROM archivo_digitales_tvd_tomos
	...
	public static function Buscar($Accion, $IdTomo, $IdDigital, $NomTomo) {
		...
		try{
			...
			if($Accion == 1){
				$Sql = "SELECT * FROM archivo_digitales_tvd_tomos
						WHERE id_tomo = :id_tomo";
