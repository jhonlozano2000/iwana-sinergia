...
class Digitalizacion{
	...
	public function Gestionar(){
		...
		try{
			if($this->Accion == 'NUEVO_EXPEDIENTE'){
				...
				$Sql = "INSERT INTO archivo_digitales_trd(id_depen, id_oficina, id_serie, id_subserie, codigo, titulo, fec_ini, fec_fin, Criterio1, Criterio2,
				...
				$Sql = "UPDATE archivo_digitales_trd SET codigo = :codigo, titulo = :titulo, fec_ini = :fec_ini, fec_fin = :fec_fin,
	...
	public static function Listar($Accion, $IdDigital, $IdDepen, $IdSerie, $IdSubSerie, $Codigo, $Titulo, $Criterio1, $Criterio2, $Criterio3){
		...
		try{
			...
			if($Accion == 1){
				...
				*********************************************************************************************/
				$Sql = "SELECT `digi`.`id_digital`, `serie`.`nom_serie`, `subserie`.`nom_subserie`, `depen`.`id_depen`, `depen`.`nom_depen`, `ofi`.`nom_oficina`,
							`digi`.`codigo`, `digi`.`titulo`, `digi`.`fec_ini`, `digi`.`fec_fin`, `digi`.`criterio1`, `digi`.`criterio2`, `digi`.`criterio3`,
							`digi`.`deposito`, `digi`.`caja`, `digi`.`carpeta`, `digi`.`folios`, `digi`.`acti`
						FROM `archivo_digitales_trd` AS `digi`
						    INNER JOIN `areas_dependencias` AS `depen` ON (`digi`.`id_depen` = `depen`.`id_depen`)
						    LEFT JOIN `areas_oficinas` AS `ofi` ON (`digi`.`id_oficina` = `ofi`.`id_oficina`)
						    INNER JOIN `archivo_trd_series` AS `serie` ON (`digi`.`id_serie` = `serie`.`id_serie`)
						    INNER JOIN `archivo_trd_subserie` AS `subserie` ON (`digi`.`id_subserie` = `subserie`.`id_subserie`)
				...
				$Sql = "SELECT `id_digital`, `codigo`, `titulo`
						FROM `archivo_digitales_trd` AS `expe`
				...
				#*********************************************************************************************
				$Sql = "SELECT * FROM archivo_digitales_trd_detalle WHERE `id_digital` = :id_digital";
				...
				$Sql = "SELECT `digi`.`id_digital`, `digi`.`id_oficina`, `digi`.`codigo`, `digi`.`titulo`, `digi_deta`.`archivo`, `digi_deta`.`id_ruta`,
						...
						FROM `archivo_digitales_trd_detalle` AS `digi_deta`
						    INNER JOIN `archivo_digitales_trd` AS `digi` ON (`digi_deta`.`id_digital` = `digi`.`id_digital`)
				...
				*********************************************************************************************/
				...
				$Sql = "SELECT `digital`.`id_digital`, `depen`.`nom_depen`, `serie`.`cod_serie`, `serie`.`nom_serie`, `subserie`.`cod_subserie`,
						...
						FROM `archivo_digitales_trd` AS `digital`
						    INNER JOIN `archivo_trd_series` AS `serie` ON (`digital`.`id_serie` = `serie`.`id_serie`)
						    INNER JOIN `archivo_trd_subserie` AS `subserie` ON (`digital`.`id_subserie` = `subserie`.`id_subserie`)
						    INNER JOIN `areas_dependencias` AS `depen` ON (`digital`.`id_depen` = `depen`.`id_depen`)
						";
				...
				*********************************************************************************************/
				...
				#*********************************************************************************************
				$Sql = "SELECT * FROM archivo_digitales_trd WHERE `id_digital` = :id_digital";
	            ...
	            $Sql = "SELECT `expe`.`id_digital`, `expe`.`titulo`, `depen`.`cod_depen`, `depen`.`cod_corres`, `depen`.`nom_depen`, `serie`.`cod_serie`,
						...
						FROM`archivo_digitales_trd` AS `expe`
						    INNER JOIN `areas_dependencias` AS `depen` ON (`expe`.`id_depen` = `depen`.`id_depen`)
						    INNER JOIN `archivo_trd_series` AS `serie` ON (`expe`.`id_serie` = `serie`.`id_serie`)
						    INNER JOIN `archivo_trd_subserie` AS `subserie` ON (`expe`.`id_subserie` = `subserie`.`id_subserie`)
	...
	public static function Buscar($Accion, $IdDigital, $IdDepen, $IdSerie, $IdSubSerie, $Codigo, $Titulo, $Criterio1, $Criterio2, $Criterio3) {
		...
		try{
			...
			if($Accion == 2){
				$Sql = "SELECT * FROM archivo_digitales_trd
