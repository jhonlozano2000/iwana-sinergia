$(document).ready(function () {
	...
	$('#BtnRadicarRecibidas').click(function () {
		...
		if ($('#fec_docu').val() == "") {
			...
			$.ajax({
				...
				type: 'POST',
				data: 'accion=GUARDAR_RADICADO&id_serie=' + $('#id_serie').val() + '&id_subserie=' + $('#id_subserie').val() + '&id_tipodoc=' + $('#id_tipodoc').val() + '&id_tipo_correspon=' + $('#id_tipo_correspon').val() + '&id_forma_llegada=' + $('#id_forma_llegada').val() + '&id_tercero=' + $('#id_tercero').val() + '&fechor_radica=' + $('#fechor_radica').val() + '&fec_docu=' + $('#fec_docu').val() + '&requie_respues=' + RequiRespues + '&fec_venci=' + $('#fec_venci').val() + '&asunto=' + $('#asunto').val() + '&num_folio=' + $('#num_folio').val() + '&num_anexos=' + NumAnexos + '&observa_anexo=' + $('#observa_anexo').val() + '&responsable=' + idResponsable + '&Destinatarios=' + Destinatarios + "&incluir_trd=" + $('#incluir_trd').val() + '&opcion_relacion=' + $('#opcion_relacion').val() + '&opcion_titulo=' + $('#opcion_titulo').val() + '&opcion_sub_titulo=' + $('#opcion_sub_titulo').val() + '&opcion_detalle1=' + $('#opcion_detalle1').val() + '&opcion_detalle2=' + $('#opcion_detalle2').val() + '&opcion_detalle3=' + $('#opcion_detalle3').val(),
				beforeSend: function () {
	...
	$(document).on('click', '#BtnEditarAsunto', function (event) {
		$.ajax({
			...
			beforeSend: function () {
				$("#DivAlertasEditarAsunto").load("../../../../config/mensajes.php", {
					alerta: 5,
					mensaje: 'Enviando información, por favor espere...'
				}, function () { });
			},
		...
		swal({
			...
			function () {
				$.ajax({
					...
					beforeSend: function () {
						$("#DivAlertasEstablecerRequiereRespues").load("../../../../config/mensajes.php", {
							alerta: 5,
							mensaje: 'Enviando información, por favor espere...'
						}, function () { });
					},
