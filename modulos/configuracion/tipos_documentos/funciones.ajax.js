$(document).ready(function () {
    ...
    $("#BtnGuardar").click(function () {
        if ($("#cod_tipo").val() == "") {
            $("#DivAlerta").load("../../../config/mensajes.php", { alerta: 3, mensaje: "Te hizo falta el código" }, function () {});
            $("#cod_tipo").focus();
        } else if ($("#nom_tipo").val() == "") {
            $("#DivAlerta").load("../../../config/mensajes.php", { alerta: 3, mensaje: "Te hizo falta el tipo de documento" }, function () {});
            $("#nom_tipo").focus();
        } else {
            ...
            $.ajax({
                ...
                data: "accion=INSERTAR&cod_tipo=" + $("#cod_tipo").val() + "&nom_tipo=" + $("#nom_tipo").val() + "&acti=" + acti,
                beforeSend: function () {
    ...
    $("#BtnEditar").click(function () {
        if (confirm("Desea editar el registro: " + $("#nom_tipo").val())) {
            if ($("#cod_tipo").val() == "") {
                $("#DivAlerta").load("../../../config/mensajes.php", { alerta: 3, mensaje: "Te hizo falta el código" }, function () {});
                $("#cod_tipo").focus();
            } else if ($("#nom_tipo").val() == "") {
                ...
                $.ajax({
                    ...
                    //Form data
                    //datos del formulario
                    data: "accion=EDITAR&id_tipo=" + $("#id_tipo").val() + "&cod_tipo=" + $("#cod_tipo").val() + "&nom_tipo=" + $("#nom_tipo").val() + "&acti=" + acti,
                    beforeSend: function () {
                        $("#DivAlerta").load("../../../config/mensajes.php", { alerta: 5, mensaje: "Enviando información...", Imagen: "../../../public/assets/img/loading.gif" }, function () {});
                    },
                    success: function (msj) {
        ...
        swal(
            ...
            function () {
                $.ajax({
                    url: "acciones.ajax.php",
                    type: "POST",
                    data: "accion=ELIMINAR&id_tipo=" + Id,
                    success: function (msj) {
                        if (msj == 1) {
                            $("#TrDatos" + Id).remove();
        ...
        swal(
            ...
            function () {
                $.ajax({
                    url: "acciones.ajax.php",
                    type: "POST",
                    data: "accion=ACTIVAR_INACTIVAR&id_despedida=" + Id + "&acti=" + Acti,
                    beforeSend: function () {
