$(document).ready(function(){
	...
	$('#BtnGuardar').click(function() {
		if ($('#nom_regimen').val() == ""){
			$("#DivAlerta").load("../../../config/mensajes.php",{alerta:3,mensaje:'Te hizo falta el nom_regimen'},function(){});
			$('#nom_regimen').focus();
		}else{
			...
			$.ajax({
				...
				data: 'accion=Insertar&nom_regimen='+$('#nom_regimen').val()+'&acti='+acti,
				beforeSend: function(){
	...
	*===========================================================================**/
	$('#BtnEditar').click(function(){
		if(confirm('Desea editar el registro: ' +  $('#nom_regimen').val())){
			if ($('#nom_regimen').val() == ""){
				...
				$.ajax({
					url: 'acciones.ajax.php',
					type: 'POST',
					data: 'accion=Editar&id_regimen='+$('#id_regimen').val()+'&nom_regimen='+$('#nom_regimen').val()+'&acti='+acti,
					beforeSend: function(){
						$("#DivAlerta").load("../../../config/mensajes.php",{alerta:5,mensaje:'Enviando información...',Imagen:'../../../public/assets/img/loading.gif'},function(){});
					},
					success:function(msj){
	...
	$(document).on('click', '#BtnEliminar', function (event){
		...
		function(){
			...
			$.ajax({
				url: 'acciones.ajax.php',
				type: 'POST',
				data: 'accion=Eliminar&id_regimen='+Id,
				success:function(msj){
					if (msj == 1){
						$("#TrDatos" + Id).remove();
						swal("¡Eliminado!",
	...
	$(document).on('click', '.acti', function(event){
		...
		function(){
			$.ajax({
				url: 'acciones.ajax.php',
				type: 'POST',
				data: 'accion=ACTIVAR_INACTIVAR&id_regimen='+Id+'&acti='+Acti,
				beforeSend: function(){
