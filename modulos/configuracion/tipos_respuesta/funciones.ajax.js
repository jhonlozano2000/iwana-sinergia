$(document).ready(function () {
    ...
    $("#BtnGuardar").click(function () {
        if ($("#nom_respues").val() == "") {
            $("#DivAlerta").load("../../../config/mensajes.php", { alerta: 3, mensaje: "Te hizo falta el tipo de respuestas" }, function () {});
            $("#nom_respues").focus();
        } else {
            ...
            $.ajax({
                ...
                data: "accion=INSERTAR&nom_respues=" + $("#nom_respues").val() + "&acti=" + acti,
                beforeSend: function () {
    ...
    $("#BtnEditar").click(function () {
        if (confirm("Desea editar el registro: " + $("#nom_respues").val())) {
            if ($("#nom_respues").val() == "") {
                $("#DivAlerta").load("../../../config/mensajes.php", { alerta: 3, mensaje: "Te hizo falta tipo de respuesta" }, function () {});
                $("#nom_respues").focus();
            } else {
                var acti = $("#acti").prop("checked") ? true : false;
                ...
                $.ajax({
                    ...
                    //Form data
                    //datos del formulario
                    data: "accion=EDITAR&id_respue=" + $("#id_respue").val() + "&nom_respues=" + $("#nom_respues").val() + "&acti=" + acti,
                    beforeSend: function () {
                        $("#DivAlerta").load("../../../config/mensajes.php", { alerta: 5, mensaje: "Enviando información...", Imagen: "../../../public/assets/img/loading.gif" }, function () {});
                    },
                    success: function (msj) {
        ...
        swal(
            ...
            function () {
                $.ajax({
                    url: "acciones.ajax.php",
                    type: "POST",
                    data: "accion=ELIMINAR&id_respue=" + Id,
                    success: function (msj) {
                        if (msj == 1) {
                            $("#TrDatos" + Id).remove();
        ...
        swal(
            ...
            function () {
                $.ajax({
                    url: "acciones.ajax.php",
                    type: "POST",
                    data: "accion=ACTIVAR_INACTIVAR&id_respue=" + Id + "&acti=" + Acti,
                    beforeSend: function () {
