$(document).ready(function(){
	...
	$('#BtnGuardar').click(function() {
		if ($('#status').val() == ""){
			$("#DivAlerta").load("../../../config/mensajes.php",{alerta:3,mensaje:'Te hizo falta el status'},function(){});
			$('#status').focus();
		}else{
			...
			$.ajax({
				...
				data: 'accion=Insertar&status='+$('#status').val()+'&acti='+acti,
				beforeSend: function(){
	...
	*===========================================================================**/
	$('#BtnEditar').click(function(){
		if(confirm('Desea editar el registro: ' +  $('#status').val())){
			if ($('#status').val() == ""){
				...
				//Form data
				//datos del formulario
				data: 'accion=Editar&id_status='+$('#id_status').val()+'&status='+$('#status').val()+'&acti='+acti,
				beforeSend: function(){
					$("#DivAlerta").load("../../../config/mensajes.php",{alerta:5,mensaje:'Enviando información...',Imagen:'../../../public/assets/img/loading.gif'},function(){});
				},
				success:function(msj){
	...
	$(document).on('click', '#BtnEliminar', function (event){
		...
		function(){
			...
			$.ajax({
				url: 'acciones.ajax.php',
				type: 'POST',
				data: 'accion=Eliminar&id_status='+Id,
				success:function(msj){
					if (msj == 1){
						$("#TrDatos" + Id).remove();
						swal("¡Eliminado!",
	...
	$(document).on('click', '.acti', function(event){
		...
		function(){
			$.ajax({
				url: 'acciones.ajax.php',
				type: 'POST',
				data: 'accion=ACTIVAR_INACTIVAR&id_status='+Id+'&acti='+Acti,
				beforeSend: function(){
