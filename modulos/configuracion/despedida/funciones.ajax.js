$(document).ready(function(){
	...
	$('#BtnGuardar').click(function() {
		if ($('#despedida').val() == ""){
			$("#DivAlerta").load("../../../config/mensajes.php",{alerta:3,mensaje:'Te hizo falta el despedida'},function(){});
			$('#despedida').focus();
		}else{
			...
			$.ajax({
				...
				data: 'accion=Insertar&despedida='+$('#despedida').val()+'&acti='+acti,
				beforeSend: function(){
	...
	*===========================================================================**/
	$('#BtnEditar').click(function(){
		if(confirm('Desea editar el registro: ' +  $('#despedida').val())){
			if ($('#despedida').val() == ""){
				...
				//Form data
				//datos del formulario
				data: 'accion=Editar&id_despedida='+$('#id_despedida').val()+'&despedida='+$('#despedida').val()+'&acti='+acti,
				beforeSend: function(){
					$("#DivAlerta").load("../../../config/mensajes.php",{alerta:5,mensaje:'Enviando información...',Imagen:'../../../public/assets/img/loading.gif'},function(){});
				},
				success:function(msj){
	...
	$(document).on('click', '#BtnEliminar', function (event){
		...
		function(){
			...
			$.ajax({
				url: 'acciones.ajax.php',
				type: 'POST',
				data: 'accion=Eliminar&id_despedida='+Id,
				success:function(msj){
					if (msj == 1){
						$("#TrDatos" + Id).remove();
						swal("¡Eliminado!",
