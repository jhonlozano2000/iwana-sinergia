$(document).ready(function(){
	...
	$('#BtnGuardar').click(function() {
		...
		if ($('#id_depen').val() == "0"){
			$("#DivAlerta").html('<div class="alert"><button class="close" data-dismiss="alert"></button><a href="#" class="link"><i class="fa fa-exclamation-circle"></i> Upsss.</a> Te hizo falta la dependencia.</div>');
			$('#id_depen').focus();
		}else if ($('#ip').val() == ""){
			...
			$.ajax({
				...
				type: 'POST',
				data: 'accion=INSERTAR&id_depen='+$('#id_depen').val()+'&ip='+$('#ip').val()+'&ruta='+$('#ruta').val()+'&usua='+$('#usua').val()+'&contra='+$('#contra').val()+'&acti='+acti,
				beforeSend: function(){
		...
		function(){
			if ($('#ip').val() == ""){
				...
				$.ajax({
					url: 'acciones.ajax.php',
					type: 'POST',
					data: 'accion=EDITAR&id_ruta='+$('#id_ruta').val()+'&ip='+$('#ip').val()+'&ruta='+$('#ruta').val()+'&usua='+$('#usua').val()+'&contra='+$('#contra').val()+'&acti='+acti,
					beforeSend: function(){
						$("#DivAlerta").html('<div class="alert alert-info"><button class="close" data-dismiss="alert"></button><a href="#" class="link"><img src="../../../public/assets/img/loading.gif" width="20" height="20"> Info.:</a> Enviando informacóm, por favor espere. </div>');
					},
					success:function(msj){
