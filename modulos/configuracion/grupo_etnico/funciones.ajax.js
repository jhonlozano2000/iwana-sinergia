$(document).ready(function(){
	...
	$('#nom_grupo_etnico').focus();

	$('#BtnGuardar').click(function() {
		if ($('#nom_grupo_etnico').val() == ""){
			$("#DivAlerta").load("../../../config/mensajes.php",{alerta:3,mensaje:'Te hizo falta el nom_grupo_etnico'},function(){});
			$('#nom_grupo_etnico').focus();
		}else{
			...
			$.ajax({
				...
				type: 'POST',
				data: 'accion=Insertar&nom_grupo_etnico='+$('#nom_grupo_etnico').val()+'&acti='+acti,
				beforeSend: function(){
	...
	*===========================================================================**/
	$('#BtnEditar').click(function(){
		if(confirm('Desea editar el registro: ' +  $('#nom_grupo_etnico').val())){
			if ($('#nom_grupo_etnico').val() == ""){
				...
				$.ajax({
					url: 'acciones.ajax.php',
					type: 'POST',
				data: 'accion=Editar&id_grupo_etnico='+$('#id_grupo_etnico').val()+'&nom_grupo_etnico='+$('#nom_grupo_etnico').val()+'&acti='+acti,
				beforeSend: function(){
					$("#DivAlerta").load("../../../config/mensajes.php",{alerta:5,mensaje:'Enviando información...',Imagen:'../../../public/assets/img/loading.gif'},function(){});
				},
				success:function(msj){
	...
	$(document).on('click', '#BtnEliminar', function (event){
		...
		function(){
			...
			$.ajax({
				url: 'acciones.ajax.php',
				type: 'POST',
				data: 'accion=Eliminar&id_grupo_etnico='+Id,
				success:function(msj){
					if (msj == 1){
						$("#TrDatos" + Id).remove();
						swal("¡Eliminado!",
	...
	$(document).on('click', '.acti', function(event){
		...
		function(){
			$.ajax({
				url: 'acciones.ajax.php',
				type: 'POST',
				data: 'accion=ACTIVAR_INACTIVAR&id_grupo_etnico='+Id+'&acti='+Acti,
				beforeSend: function(){
