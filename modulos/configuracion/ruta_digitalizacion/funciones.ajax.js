	...
	$('#BtnGuardar').click(function() {
		if ($('#ip').val() == ""){
			...
			$.ajax({
				...
				type: 'POST',
				data: 'accion=INSERTAR&servidor='+$('#servidor').val()+'&ruta='+$('#ruta').val()+'&usua='+$('#usua').val()+'&contra='+$('#contra').val()+'&acti='+acti,
				beforeSend: function(){
		...
		function(){
			if ($('#ip').val() == ""){
				...
				$.ajax({
					url: 'acciones.ajax.php',
					type: 'POST',
					data: 'accion=EDITAR&id_ruta='+$('#id_ruta').val()+'&servidor='+$('#servidor').val()+'&ruta='+$('#ruta').val()+'&usua='+$('#usua').val()+'&contra='+$('#contra').val()+'&acti='+acti,
					beforeSend: function(){
						$("#DivAlerta").html('<div class="alert alert-info"><button class="close" data-dismiss="alert"></button><a href="#" class="link"><img src="../../../public/assets/img/loading.gif" width="20" height="20"> Info.:</a> Enviando informacóm, por favor espere. </div>');
					},
					success:function(msj){
