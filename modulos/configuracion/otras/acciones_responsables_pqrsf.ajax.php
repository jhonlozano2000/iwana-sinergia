...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	switch ($Accion) {
		case 'INSERTAR_RESPONSABLE':
			...
			$BuscarResponsables = ConfigOtrasResponsablePQRSF::Buscar(1, $IdFuncio);
			if ($BuscarResponsables) {
				echo "Este funcionario ya se encuentra como funcionario responsable de las solicitudes de PQRSF.";
				exit();
			}

			$Resposable = new ConfigOtrasResponsablePQRSF();
