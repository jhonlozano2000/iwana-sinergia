...
if (! empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	require_once '../../../clases/oficina_archivo/class.OficinaArchivoDigitalizacionTVDArchivos.php';
	require_once '../../../clases/oficina_archivo/class.OficinaArchivoDigitalizacionTVDTomo.php';
	...
	switch($Accion){
		case 'NUEVO_EXPEDIENTE':
			...
			*/
			$Digitalizado = new DigitalizacionTVD();
			...
			if($Digitalizado->Gestionar() == true){
				...
				/* CREO EL PRIMER TOMO
				/********************************************************************************/
				$TotalTomo = DigitalizacioTVDTomo::Listar(1, "", $Digitalizado->get_IdDigital(), "");
				$NomTomo = $TotalTomo['TotalTomos']+1;
				$Tomo = new DigitalizacioTVDTomo();
				...
				$Servidor = ServidorDigitalizacion::Buscar(5, 0, "", "");
				...
				if($ftpObj->pr($ftpObj->getMessages()[0]) == 'true'){
					...
					if(count($_FILES["file"]["name"]) > 0){
						...
						$DigitalArchivos = new DigitalizacionTVDArchivos();
		...
		case 'SUBIR_ARCHIVOS':
			...
			if($Digitalizado->Gestionar() == true){
				...
				/********************************************************************************
				/* BUSCO EL TOMO EN EL Q SE VAN A SUBIR LOS ARCHIVOS
				/********************************************************************************/
		...
		case 'ELIMINAR_DIGITAL':
			...
			if($ftpObj->pr($ftpObj->getMessages()[0]) == 'true'){
				...
				if($ftpObj->pr($ftpObj->getMessages()[0]) == 'true'){
					...
					DigitalizacionTVD::Gestionar_Archivos(2, $IdArchivoDigital, "", "", "" , "", "", "", "", "", "");
		...
		break;
		case 'NUEVO_TOMO':
			$TotalTomo = DigitalizacioTVDTomo::Listar(1, "", $IdDigital, "");
