$(document).ready(function(){
	...
	$("#id_depen").change(function(){
		...
		if($("#incluir_oficina_trd").val() == 1){
			...
			$.post("../../../varios/combo_series_tvd.php", {
				id_depen: IdDepen,
				id_oficina: '',
	...
	$("#id_serie").change(function () {
		if($("#id_serie").val() != 0){
			$.post("../../../varios/combo_sub_series_tvd.php", {
				id_serie: $(this).val(),
				id_depen: $('#id_depen').val(),
	...
	$(document).on('click', '#BtnDescargaArchivo', function(event){
		...
		$.ajax({
			...
			type: 'POST',
			data: 'accion=DESCARGAR_ARCHIVO_EXPEDIENTE_DIGITAL_TVD&id_digital='+IdDigital+'&id_archivo='+IdArchivo+'&id_ruta='+IdRuta+'&archivo='+Archivo+'&id_tomo='+IdTomo,
			beforeSend: function(){
