...
switch ($Accion){
	case 'INSERTAR':
		...
		$BuscarSubSerie = SubSerieTVD::Buscar(2, "", $CodSubSerie, $NomSubSerie);
		if($BuscarSubSerie){
			echo "La Subserie ".$NomSubSerie." ya se encuentra registrada en el sistema.";
			exit();
		}

		$SubSerie = new SubSerieTVD();
		...
		if($SubSerie -> Gestionar() == true){
			for($i=0;$i<count($TiposDocumentos); $i++){
				...
				$BuscarTipoDocumento = SubSerieDetalleTVD::Buscar(1, $SubSerie->getId_SubSerie(), $TiposDocumentos[$i]);
